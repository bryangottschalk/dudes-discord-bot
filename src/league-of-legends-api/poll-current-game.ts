...
let isPollIterationInProgress = false;
...
const handleLoLClientEvent = async (
  newEvent: Event,
  ...
  switch (newEvent?.EventName) {
    ...
    case LoLClientEvent.DRAGON_KILLED: {
      if (newEvent.Stolen === 'True') {
        ...
        await playRandomClipFromFolder(EventFiles.LOL_OBJECTIVE_STEAL, channel, audioPlayer);
      } else {
    ...
    case LoLClientEvent.VOIDGRUB_KILLED: {
      if (newEvent.Stolen === 'True') {
        ...
        await playRandomClipFromFolder(EventFiles.LOL_VOIDGRUB_KILLED, channel, audioPlayer);
      }
      break;
    }
...
export const startPollingLoLGame = (channel: VoiceBasedChannel, audioPlayer: AudioPlayer) => {
  ...
  timeoutManager.updateChannel(channel, audioPlayer);
  ...
  if (leagueOfLegendsPollTimer === null) {
    ...
    leagueOfLegendsPollTimer = setIntervalImmediately(async () => {
      if (isPollIterationInProgress) {
        return;
      }

      isPollIterationInProgress = true;
      ...
      try {
        ...
        const newEvents =
          currentEvents.length >= cachedEvents.length
            ? currentEvents.slice(cachedEvents.length)
            : currentEvents;
        ...
        for (const newEvent of newEvents) {
          ...
          const shouldStopPolling = await handleLoLClientEvent(newEvent, channel, audioPlayer);
        ...
        isPollIterationInProgress = false;
      }
